...
CDrawScene::CDrawScene(Ccg2022QB3DTransView* pView)
{
	m_pView = pView;
	...
	glObjColor[CUBE][0] = 1.0f;  glObjColor[CUBE][1] = 0.0f;  glObjColor[CUBE][2] = 0.0f;
...
{
	int forLimitValue = 3;
	Ccg2022QB3DTransDoc *pDoc = (Ccg2022QB3DTransDoc *)m_pView->GetDocument();
...
void CDrawScene::CaculateMatrix()
{
	...
	case 0: // -X Trans
		...
		case 0: // translate
			...
			break;
		case 1: // rotate
			rotateX3Dmatrix(-sinf(THETA), cosf(THETA));
			break;
		}
		break;
	case 1: // +X Trans
	...
	case 2: // -Y Trans
		...
		case 0: // translate
			...
			break;
		case 1: // rotate
			rotateY3Dmatrix(-sinf(THETA), cosf(THETA));
			break;
		}
		break;
	case 3: // +Y Trans
	...
	case 4:  // +Z Trans
		...
		case 0: // translate
			...
			break;
		case 1: // rotate
			rotateZ3Dmatrix(sinf(THETA), cosf(THETA));
			break;
		}
		break;
	case 5:  // -Z Trans
...
void CDrawScene::translate3dMatrix(float Tx, float Ty, float Tz)
{
	...
	// Please fill the right codes below.
	......
}
...
void CDrawScene::pRemoveBackFace()
{
	...
	for (i = 0; i < m_whoObject->polyCount; i++) {
		...
		dx1 = m_whoObject->objectSpace[i].transObject[2].x - m_whoObject->objectSpace[i].transObject[1].x;
		dy1 = m_whoObject->objectSpace[i].transObject[2].y - m_whoObject->objectSpace[i].transObject[1].y;
		dz1 = m_whoObject->objectSpace[i].transObject[2].z - m_whoObject->objectSpace[i].transObject[1].z;
		dx2 = m_whoObject->objectSpace[i].transObject[1].x - m_whoObject->objectSpace[i].transObject[0].x;
		dy2 = m_whoObject->objectSpace[i].transObject[1].y - m_whoObject->objectSpace[i].transObject[0].y;
		dz2 = m_whoObject->objectSpace[i].transObject[1].z - m_whoObject->objectSpace[i].transObject[0].z;
		nDx = dy1 * dz2 - dy2 * dz1;
		nDy = dx2 * dz1 - dx1 * dz2;
		nDz = dx1 * dy2 - dx2 * dy1;
